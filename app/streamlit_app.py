...
class TravelApp:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar.expander("🧪 OpenAi Settings", expanded=True):
            temperature = st.slider("Temperature Rate", 0.0, 2.0, 0.7, 0.1)
            top_p = st.slider("Top P Rate", 0.0, 2.0, 0.7, 0.1)
    ...
    def run(self):
        try:
            ...
            input_type = st.selectbox("Como deseja pesquisar?", ["Selecione", "Arquivo", "Texto"])

            if input_type == "Texto":
                with st.form("text_input_form"):
                    st.write("Digite o nome para obter os pontos turísticos.")    
                    hotel = st.text_input("Hotel Name", placeholder="Name of your hotel (e.g., Hotel XYZ)", key="hotel_text")
                    destination = st.text_input("Destination", placeholder="e.g., Paris, France", key="destination_text")
                    submitted = st.form_submit_button("Submit")
                if submitted and destination:
                    st.session_state.df = self.controller.handle_text_input(destination, start_date, end_date, temperature, top_p)
                    self.df = st.session_state.df
                    st.data_editor(self.df, num_rows="dynamic", use_container_width=True)
                ...
                with st.form("file_input_form"):
                    ...
                    hotel = st.text_input("Hotel Name", placeholder="Name of your hotel (e.g., Hotel XYZ)", key="hotel_file")
                    st.code(
                    ...
                    if submitted and uploaded_file:
                        try:
                            st.session_state.df = self.controller.handle_file_upload(uploaded_file)
                            self.df = st.session_state.df
                            st.data_editor(self.df, num_rows="dynamic", use_container_width=True)
